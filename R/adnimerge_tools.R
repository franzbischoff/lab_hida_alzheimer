summary.default <- function(object, ..., digits = max(3, getOption("digits") -
  ...
  value <- if (is.logical(object)) {
    ...
    qq <- signif(c(
      N = N, qq[1:3], mean(object), qq[4:5],
      SD = sd(object), SE = sd(object) / sqrt(N)
    ), digits)
    names(qq) <- c(
      "N", "Min.", "Q.1", "Median", "Mean",
      "Q.3", "Max.", "SD", "SE"
    )
    ...
    (n <- length(object))) {
    sumry <- array("", c(n, 3), list(names(object), c(
      "Length",
      "Class", "Mode"
    )))
    ll <- numeric(n)
...
summary2matrix <- function(x, hsep = "|", header.vsep = FALSE, ...) {
  tf <- tempfile()
  trash <- capture.output(print(x,
    hsep = hsep, header.vsep = FALSE,
...
longsums <- function(Data, oc = "ADAS13") {
  ...
  dd1 <- merge(subset(dd, RID > 2000 & VISCODE == "bl", c("RID", "DX")),
    Data[Data$RID > 2000, c("RID", "VISCODE", oc)],
    by = "RID", all.y = TRUE
  )
  dd1$Month <- ifelse(dd1$VISCODE %in% c("sc", "bl"), 0, as.numeric(gsub("m", "", dd1$VISCODE)))
  dd1$Y <- dd1[, oc]
  dd1 <- merge(dd1, subset(dd1, Month == 0, c("RID", "Y")),
    by = "RID",
  ...
  latex(tab[, -(1:2)],
    file = "", rowlabel = "Month", rowname = tab[, 1],
...
make_first_to_final_trans <- function(RID, stage, Year) {
  ...
  for (rid in unique(dd$RID)) {
    ...
    nr <- nrow(subdd)
    ddf <- rbind(ddf, c(
      id = rid, start = subdd$Year[1], stop = subdd$Year[nr],
    ...
    if (nrow(subdd) > 1) {
      ...
      for (j in 1:(nrow(subdd) - 1)) {
        if (subdd$stage[j] != start.stage) {
          dd2 <- rbind(dd2, c(
            id = rid, start = start, stop = subdd$Year[j],
      ...
      dd2 <- rbind(dd2, c(
        id = rid, start = start, stop = subdd$Year[j + 1],
...
my_msSurv_plot <- function(x, states = "ALL", trans = "ALL", plot.type = "stateocc",
                           ...
                           legend.title = "Stage", ...) {
  require(ggplot2)
  plot.type <- match.arg(plot.type, c(
    "stateocc", "transprob", "entry.sub",
  ...
  if (plot.type == "stateocc") {
    ...
    if (CI == TRUE & !is.null(var.sop(x))) {
      ...
      st.plot <- update(st.plot,
        main = "Plot of State Occupation Probabilites",
        xlab = "Event Times", ylab = "State Occupation Probabilities",
  ...
  if (plot.type == "transprob") {
    ...
    if (CI == TRUE & !is.null(cov.AJs(x))) {
      ...
      tr.plot <- update(tr.plot,
        main = "Plot of Transition Probabilites",
  ...
  if (plot.type == "entry.sub") {
    ...
    if (CI == TRUE & !is.null(Fsub.var(x))) {
      ...
      xvals <- rep(et(x), length(states))
      f.st <- as.factor(rep(dimnames(rd)$state[sl], each = dim(rd)[1]))
      ent.plot <- xyplot(y + y2 + y3 ~ xvals | f.st,
        allow.multiple = TRUE,
        type = "s", lty = c(1, 2, 2), col = c(1, 2, 2), ...
      )
      ent.plot <- update(ent.plot,
        main = "Plot of State Entry Time Subdistributions",
        xlab = "Event Times", ylab = "State Entry Time Subdistributions",
        key = list(
          lines = list(col = c(1, 2, 2), lty = c(1, 2, 2)),
          text = list(c("Est", "Lower CI", "Upper CI")),
          columns = 3
        )
      )
  ...
  if (plot.type == "entry.norm") {
    ...
    if (CI == TRUE & !is.null(Fnorm.var(x))) {
      ...
      ent.plot <- update(ent.plot,
        main = "Plot of Normalized State Entry Time Distributions",
  ...
  if (plot.type == "exit.sub") {
    ...
    if (CI == TRUE & !is.null(Gsub.var(x))) {
      ...
      xvals <- rep(et(x), length(states))
      f.st <- as.factor(rep(dimnames(rd)$state[sl], each = dim(rd)[1]))
      exit.plot <- xyplot(y + y2 + y3 ~ xvals | f.st,
        allow.multiple = TRUE,
        type = "s", lty = c(1, 2, 2), col = c(1, 2, 2), ...
      )
      exit.plot <- update(exit.plot,
        main = "Plot of State Exit Time Distributions",
...
Add.start <- function(Data) {
  ...
  new.data <- data.frame(
    id = Data$id, start = Data$start, stop = Data$stop,
...
LT.Data <- function(Data) {
  ...
  ## dummy initial stage
  dummy <- data.frame(
    id = ids, start = -1, stop = stop.time, start.stage = "LT",
...
CP <- function(tree, tree0, Data, nt.states) {
  ...
  ds <- paste(
    "dN", rep(nodes(tree0)[nodes(tree0) %in% nt.states], lng),
...
AJ.estimator <- function(ns, tree, dNs.et, Ys.et, start.probs) {
  ...
  all.dA <- all.I.dA <- all.AJs <- array(
    dim = c(ns, ns, nrow(dNs.et)),
    dimnames = list(
      rows = nodes(tree),
...
BS.var <- function(Data, tree, ns, et, cens.type, B, LT) {
  ...
  bs.est <- array(
    dim = c(length(nodes(tree)), length(nodes(tree)), length(et), B),
  ...
  bs.Fnorm <- bs.Fsub <- bs.ps
  bs.Gnorm <- bs.Gsub <- bs.ps ## storage for BS entry/exit
  ...
  for (b in 1:B) { ## randomly selects the indices
    ...
    cp.red <- Red(
      tree, cp$dNs, cp$Ys, cp$sum_dNs, ds.est$dNs.K,
  ...
  list(
    cov.AJs = bs.cov, var.sop = bs.var.sop, cov.dA = bs.IdA.cov,
...
MSM.CIs <- function(x, ci.level = 0.95, ci.trans = "linear", trans = TRUE, sop = TRUE) {
  ...
  if (sop == TRUE) {
    CI.p <- array(0,
      dim = c(nrow(dNs(x)), 3, length(nodes(tree(x)))),
      dimnames = list(
        rows = et(x),
  ...
  if (trans == TRUE) {
    CI.trans <- array(0,
      dim = c(nrow(dNs(x)), 4, length(pos.trans(x))),
      dimnames = list(
        rows = et(x),
...
Dist.CIs <- function(x, ci.level = 0.95, ci.trans = "linear", norm = TRUE) {
  ...
  CI.Fs <- array(0,
    dim = c(length(et(x)), 3, length(nodes(tree(x)))),
    dimnames = list(
      rows = et(x), cols = c("est", "lower limit", "upper limit"),
